...
class LeaveReportEditRepository {
  Future<LeavereportEditModel> getLeaveReportEdit(int ids) async {
    final Map<String, dynamic> _queryParameters = <String, dynamic>{};
    print("chart/events/${ids}");
    final response = await WebApiProvider().getData(
        url: "chart/events/${ids}",
        isPost: false,
        token: TOKEN,
        queryParameters: _queryParameters,
        isQueryParmeter: false);
    print("data->>>>>>>>>${response}");
    return LeavereportEditModel.fromJson(response);
  }
}
